...
namespace projektarbete_grupp42.Tests
{
    [TestClass]
    public class IntegrationTests
    {
        ...
        //vår metod ang. change kollar så att change byter lösenord och använder get för att kolla att det stämmer
        [TestMethod]
        public void ChangeCommandChangesPassword()
        {
            string oldPwrd = "oldPassword";
            string newPwrd = "newPassword";
            string key = "testkey";
            string savedValue = "savedValue";

            init(clientPath1, serverPath, oldPwrd);
            set(clientPath1, serverPath, oldPwrd, key, savedValue);

            string changePwrd = change(clientPath1, serverPath, oldPwrd, newPwrd);
            StringAssert.Contains(changePwrd, "Password changed");

            string newPwrdWork = get(clientPath1, serverPath, newPwrd, key).Split(Environment.NewLine)[^1];
            Assert.AreEqual(savedValue, newPwrdWork, "get works with new password");

            string oldPwrdWork = get(clientPath1, serverPath, oldPwrd, key);
            Assert.IsFalse(oldPwrdWork.Contains(savedValue), "get does not work with old password");
            StringAssert.Contains(oldPwrdWork, "Could not decrypt");
        }
        ...
        // vår hjälp metod för change
        private string change(string clientPath, string serverPath, string currentPwrd, string newPwrd)
        {
            resetConsoleOutput();
            setConsoleInput(currentPwrd, newPwrd);
